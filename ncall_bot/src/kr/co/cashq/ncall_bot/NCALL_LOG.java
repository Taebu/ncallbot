...
public class NCALL_LOG {
	...
	public static void main(String[] args) {
		...
		Utils.getLogger().info("NCALL_LOG program started!");
		try {
			...
			Utils.getLogger().info("NCALL_LOG program ended!");
		} catch (Exception e) {
					...
					if (0 < heart_beat++) {
						if (DBConn.getConnection() != null) {
							...
							if (heart_beat < Env.MINUTE) {
								this_thread.wait(1000*60*60*1);// 1시간 대기 (유닉스*초*분*시간)
							} else if (heart_beat < Env.MINUTE * 2) {
								this_thread.wait(1000*60*60*2);//  2시간 대기 (유닉스*초*분*시간)
							} else if (heart_beat < Env.MINUTE * 4) {
								this_thread.wait(1000*60*60*3);//  3시간 대기 (유닉스*초*분*시간)
							} else if (heart_beat < Env.MINUTE * 8) {
								this_thread.wait(1000*60*60*4); //  4시간 대기 (유닉스*초*분*시간)
							} else {// 그 외는 5초 대기
								this_thread.wait(1000*60*60*5);//  5시간 대기 (유닉스*초*분*시간)
							}

							if (log60Minute()) {
								Utils.getLogger().info("NCALL_LOG alive");
							}
	 ...
	 * @return
	 */
	private static boolean log60Minute() {
		boolean retVal = false;
		long ctime = System.currentTimeMillis();
		if (loggedTime < ctime) {
			loggedTime = ctime + N3600000;
			retVal = true;
		}
		return retVal;
	}
...
//			strRetVal = w.doMsgMain();
	...
	private static boolean terminate_this() {
		...
		File file = new File(NCALL_LOG.strPrePath + "killme.txt");
